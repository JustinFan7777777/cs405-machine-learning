...
# Detector 模块说明（中文注释）
# 这个文件实现了 `Detector` 类，它负责：
# - 从磁盘或内存加载训练好的分类器（包含 scaler 和描述符参数）
# - 基于滑动窗口对单帧图像进行候选窗口分类
# - 将若干帧的候选检测框累积到热图（heatmap）中，通过加权求和
#   对频繁被判为正样本的像素加权更高，从而减少单帧误检
# - 对累积热图应用阈值并用连通域标记（label），再提取每个连通域
#   的最大外接矩形作为最终检测框并绘制在视频帧上
...
class Detector:
    ...
    def __init__(self, init_size=(64, 64), x_overlap=0.5, y_step=0.05,
        ...
        # 注意：self.windows 会在 detectVideo 内根据输入帧尺寸用 slidingWindow
        # 生成并缓存全局候选窗口列表，以便在每帧重用（避免重复计算）。
    ...
    def loadClassifier(self, filepath=None, classifier_data=None):
        ...
        # 说明：这里并不直接将 descriptor 对象反序列化并赋值给 self.descriptor，
        # 而是使用训练时保存的参数重新构造一个 Descriptor 实例。这能避免
        # 反序列化时出现不可序列化对象导致的错误（尤其是当 descriptor 内
        # 部含有 OpenCV 对象时）。
    ...
    def classify(self, image):
        ...
        # 返回被判为正样本（predictions==1）的窗口坐标列表
    ...
    def detectVideo(self, video_capture=None, num_frames=9, threshold=120,
        ...
        if write:
            if output_filename is None:
                ...
                vidFilename = output_filename
                ext = os.path.splitext(vidFilename)[1].lower()
                if ext == ".mp4":
